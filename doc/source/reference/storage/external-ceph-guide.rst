...
or similar.  The keyrings and ceph.conf generated by the Ceph CLI will have lines that start with
tabs, so make sure to remove these tabs before deploying because Kolla Ansible will throw ini parsing
errors otherwise.
